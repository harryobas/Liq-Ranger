pub mod aave_config;
pub mod aave_liquidator;
pub mod aave_watchlist;
pub mod abi_bindings;
pub mod helpers;
pub mod types;
pub mod watchlist_updater;

use std::sync::Arc;

use aave_config::AaveConfig;
use aave_liquidator::AaveLiquidator;
use aave_watchlist::AaveWatchList;
use watchlist_updater::AaveWatchListUpdater;

use crate::{
    aave::abi_bindings::{AaveOracle, IAaveV3Pool, UiPoolDataProvider},
    common::{
        self, abi_bindings::IFlashLiquidator, task_manager::spawn_named_and_register, AdminCmd,
        Config, Liquidator,
    },
};
use ethers::providers::Middleware;
use tokio::sync::{mpsc, watch};

pub async fn start_engine<M: Middleware + 'static>(
    client: Arc<M>,
    shutdown_rx: watch::Receiver<bool>,
    prune_rx: mpsc::Receiver<AdminCmd>,
    watch_list: Arc<AaveWatchList>,
    contracts: (
        IAaveV3Pool<M>,
        AaveOracle<M>,
        UiPoolDataProvider<M>,
        IFlashLiquidator<M>,
    ),
) -> anyhow::Result<Arc<dyn Liquidator>> {
    let mut aave_config = AaveConfig::load()?;

    match aave_config.populate_vdebt_tokens(client.clone()).await {
        Ok(_) => tracing::info!("✅ Aave  vdebt tokens populated successfully"),
        Err(e) => {
            tracing::error!("❌ Failed to populate Aave vdebt tokens: {:?}", e);
            return Err(anyhow::anyhow!("Failed to populate Aave vdebt tokens"));
        }
    }

    let aave_config = Arc::new(aave_config);

    let (pool, oracle, pool_data, flash_liq) = contracts;

    let aave_liq = AaveLiquidator::new(
        aave_config.clone(),
        client.clone(),
        watch_list.clone(),
        pool.clone(),
        oracle.clone(),
        pool_data.clone(),
        flash_liq.clone(),
    );

    spawn_named_and_register("aave_watchlist_updater", async move {
        let aave_updater = AaveWatchListUpdater::new(
            watch_list.clone(),
            Arc::new(pool.clone()),
            aave_config.clone(),
            shutdown_rx.clone(),
            prune_rx,
        );

        if let Err(e) = aave_updater.start().await {
            tracing::error!("❌ Aave watch list updater failed: {:?}", e);
        }
    })
    .await;

    Ok(Arc::new(aave_liq))
}
