// src/infrastructure/adapters/protocol_readers/morpho_math_helpers.rs
...
pub fn seized_assets_from_repaid_shares(
    ...
    mul_div_down(wmul_down(repaid_assets, lif), oracle_price_scale, price)
}
...
pub fn repaid_assets_from_seized_collateral(
    ...
    let lif = incentive_factor(lltv);
    let repaid_shares = to_shares_up(
        wdiv_up(quoted, lif),
        total_borrow_assets,
        total_borrow_shares,
    );
...
mod tests {
    ...
    fn share_asset_conversions_include_virtual_offsets() {
        ...
        let computed_assets = to_assets_down(shares, total_assets, total_shares);
        assert_eq!(
            to_shares_down(computed_assets, total_assets, total_shares),
            shares
        );
    }
    ...
    fn to_assets_up_rounds_up() {
        let res = to_assets_up(U256::from(1), U256::from(100), U256::from(1_000_000));
    ...
    fn repaid_assets_from_shares_rounds_up() {
        ...
        let down = to_assets_down(shares, total_assets, total_shares);
        let up = repaid_assets_from_repaid_shares(shares, total_assets, total_shares);
    ...
    fn seized_from_repaid_shares_is_at_most_theoretical() {
        ...
        let theoretical = mul_div_down(
            wmul_down(
                ...
                incentive_factor(lltv),
            ),
            scale,
            price,
        );
        let seized = seized_assets_from_repaid_shares(
            shares,
            total_assets,
            total_shares,
            lltv,
