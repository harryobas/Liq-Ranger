...
pub static USDC: Lazy<Address> =
    Lazy::new(|| Address::from_str("0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359").expect("Failed"));
...
pub static PRIVATE_KEY: Lazy<SecretString> =
    Lazy::new(|| SecretString::new(load_private_key().into()));

pub static RPC_URL: Lazy<String> = Lazy::new(|| load_rpc_url());

pub static RPC_URL_HTTP: Lazy<String> = Lazy::new(|| match env::var("RPC_URL_HTTP") {
    Ok(url) => url,
...
pub static GLOBAL_TASK_HANDLES: Lazy<Mutex<Vec<JoinHandle<()>>>> =
    Lazy::new(|| Mutex::new(Vec::new()));

// Morpho
pub static MORPHO_BLUE: Lazy<Address> =
    Lazy::new(|| Address::from_str("0x1bF0c2541F820E775182832f06c0B7Fc27A25f67").expect("Failed"));
pub const VIRTUAL_ASSETS: u128 = 1;
pub const VIRTUAL_SHARES: u128 = 1_000_000;

pub static WAD: Lazy<U256> = Lazy::new(|| pow10(18));

pub static MAX_LIQUIDATION_INCENTIVE_FACTOR: Lazy<U256> =
    Lazy::new(|| max_liquidation_incentive_factor());

pub static ORACLE_PRICE_SCALE: Lazy<U256> = Lazy::new(|| oracle_price_scale());

pub static LIQUIDATION_CURSOR: Lazy<U256> = Lazy::new(|| liquidation_cursor());
...
pub static UIPOOL_DATA_PROVIDER: Lazy<Address> =
    Lazy::new(|| Address::from_str("0xFa1A7c4a8A63C9CAb150529c26f182cBB5500944").expect("Failed"));
pub static AAVE_V3_POOL: Lazy<Address> =
    Lazy::new(|| Address::from_str("0x794a61358D6845594F94dc1DB02A252b5b4814aD").expect("Failed"));
pub static AAVE_ORACLE: Lazy<Address> =
    Lazy::new(|| Address::from_str("0xb023e699F5a33916Ea823A16485e259257cA8Bd1").expect("Failed"));
pub static POOL_ADDRESS_PROVIDER: Lazy<Address> =
    Lazy::new(|| Address::from_str("0xa97684ead0e402dC232d5A977953DF7ECBaB3CDb").expect("Failed"));
